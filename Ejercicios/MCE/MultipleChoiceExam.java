...
public class MultipleChoiceExam extends JFrame {
	...
	JRadioButtonMenuItem("Header 1", true); JRadioButtonMenuItem
	header2MenuItem = new JRadioButtonMenuItem("Header 2", false);
	...
	Font
	examItemFont = new Font("Arial", Font.BOLD, 16);
    ...
    public MultipleChoiceExam() {
    	...
    	for (int i= 0; i < 4; i++){
    			...
    			commentTextArea.setFont(new Font("Courier New", Font.BOLD +
    			Font.ITALIC, 18));
    			...
    			tipoGroup.add(mcMenuItem);
    			tipoGroup.add(typeMenuItem);
    			...
    			setBounds((int) (0.5*(screenSize.width - getWidth())), (int) (0.5*(screenSize.height - getHeight())), getWidth(), getHeight()); // initialize form
    			startButton.setEnabled(false);
    			nextButton.setEnabled(false);
    			optionsMenu.setEnabled(false);
    			commentTextArea.setText(centerTextArea("Open Exam File toStart")); }
    			private void exitForm(WindowEvent evt)
    			{
    			System.exit(0);
    			}
    			...
    			int labelSelected;
    			// make sure exam has started and question has not been answered if
    			...
    			answerLabel[labelSelected].getY()) break;
    			}
    			...
    			new DecimalFormat("0.0").format(100.0 * ((double) numberCorrect /
    			numberTried)) + "%"; JOptionPane.showConfirmDialog(null, message,
    			...
    			commentTextArea.setText(centerTextArea("ChooseOptions\nClick Start Exam")); fileMenu.setEnabled(true);
    			...
    			FileNameExtensionFilter("Exam Files", "csv")); if
    			(openChooser.showOpenDialog(this) ==
    			JFileChooser.APPROVE_OPTION) {
    			try
    			{
    			BufferedReader inputFile = new BufferedReader(new
    			FileReader(openChooser.getSelectedFile())); myLine =
    			inputFile.readLine();
    			...
    			while (inputFile.ready() && numberTerms < 100); if
    			(numberTerms < 5)
    			{
    			...
    			this.setTitle("Multiple Choice Exam - " + examTitle); // set up menu items
    			header1MenuItem.setText(header1 + ", Given " + header2);
    			header2MenuItem.setText(header2 + ", Given " + header1); if
    			(header1MenuItem.isSelected())
    			{
    			...
    			// Display multiple choice answers in label boxes if
    			if(header1MenuItem.isSelected())
    			{
    			...
    			commentTextArea.setText(centerTextArea("Sorry ... CorrectAnswer Shown")); // Display correct answer
    			if (mcMenuItem.isSelected())
    			{
    			if (header1MenuItem.isSelected())
    			...
    			{
    			// Generates Soundex code for W based on Unicode value // Allowsanswers whose spelling is close, but not exact 
    				String wTemp, s = "";
    			int l;
    			int wPrev, wSnd, cIndex;
    			// Load soundex function array
    			int[] wSound = {0, 1, 2, 3, 0, 1, 2, 0, 0, 2, 2, 4, 5, 5, 0, 1, 2, 6, 2, 3, 0, 1, 0,
    			2, 0, 2}; wTemp = w.toUpperCase();
    			l = w.length();
    			if (l != 0)
    			{
